...
async function main() {
  ...
  const realEvidenceGolden = [
    ...
    `09-18 03:30:47.983 I/ReactNativeJS(30901): KALSA_SESSION {"op":"save","ms":156,"ok":true,"estimatedBytes":36116476,"usedTokens":5359,"tokens":5359,"hash":"3666333890","messageCount":21,"stemHash":"<redacted>"}`,
    ...
    `09-18 03:31:08.640 I/ReactNativeJS(32497): KALSA_SESSION {"op":"load","ms":129,"ok":true,"tokensOnDisk":5359,"tokens":5359,"stemHash":"<redacted>"}`,
    ...
    `09-18 03:31:12.956 I/ReactNativeJS(32497): KALSA_SESSION {"op":"save","ms":223,"ok":true,"estimatedBytes":36116476,"usedTokens":5359,"tokens":5359,"hash":"3666333890","messageCount":21,"stemHash":"<redacted>"}`,
    `09-18 03:31:33.148 I/KALSA_RP_MARK(  388): fg_kick cycle=2`,
    `09-18 03:31:33.307 I/ReactNativeJS(32497): KALSA_SESSION {"op":"save","ms":1,"ok":true,"estimatedBytes":36116476,"usedTokens":5359,"reason":"unchanged","stemHash":"<redacted>"}`,
    `09-18 03:33:47.275 I/KALSA_RP_MARK( 1029): fg_settled cycle=2`,
    `09-18 03:33:52.221 I/ReactNativeJS(32497): KALSA_SESSION {"op":"save","ms":3,"ok":true,"estimatedBytes":36116476,"usedTokens":5359,"reason":"unchanged","stemHash":"<redacted>"}`,
    `09-18 03:33:52.316 I/ReactNativeJS(32497): KALSA_SESSION {"op":"save","ms":2,"ok":true,"estimatedBytes":36116476,"usedTokens":5359,"reason":"unchanged","stemHash":"<redacted>"}`,
    ...
    `09-18 03:35:28.160 I/ReactNativeJS(32497): KALSA_SESSION {"op":"save","ms":128,"ok":true,"estimatedBytes":38371612,"usedTokens":5697,"tokens":5697,"hash":"2526768741","messageCount":23,"stemHash":"<redacted>"}`,
  ].join("\n");
  ...
  // Bounded, and the bound is validated like the other bounce flags. Wall
  // clock for the same reason rp_fg_wait_settled's is: a pass counter
  // reported with an "s" suffix under-counts the budget.
  assert(
    launchWaitFn.includes("t0=$(date +%s)") &&
      launchWaitFn.includes("deadline=$((t0 + LAUNCH_PREWARM_TIMEOUT_SECONDS))") &&
      launchWaitFn.includes("settled after $(( $(date +%s) - t0 ))s"),
    "the launch wait is bounded by LAUNCH_PREWARM_TIMEOUT_SECONDS and times against the wall clock",
  );
  ...
  // The wait's terminal-op pattern, extracted from the shipped awk snippet
  // and exercised against the REAL wire shapes: React Native's two-argument
  // console.log reaches logcat as  I/ReactNativeJS(pid): 'KALSA_PREWARM',
  // '{...}'  — single quotes and a comma, never clean JSON, so the pattern
  // must match mid-line and must never anchor to end-of-line. The snippet
  // reads the capture in awk because `grep -q` on a pipe is the SIGPIPE
  // false-negative the protocol forbids (pinned above).
  const ereMatch = launchWaitFn.match(/\/([^/]+)\/ \{ found/);
  assert(ereMatch, "the launch wait polls the capture for a terminal prewarm op");
  ...
  const runValidate = (settle) =>
    spawnSync("bash", [validateFile], {
      ...
      env: {
        ...
        // rp_validate_bounce_flags validates the launch-prewarm budget
        // beside the settle one; the probe must supply every flag it reads.
        LAUNCH_PREWARM_TIMEOUT_SECONDS: "120",
      },
    });
