...
# Backup
cp scripts/bench/benchGraders.mjs scripts/bench/benchGraders.mjs.test_backup
...
const file = 'scripts/bench/benchGraders.mjs';
...
echo ""
node scripts/mutation/mutation_test.mjs 2>&1 | grep -E "(Test [0-9]|Input:|fact_|PASS|FAIL|abstained|FOUND)" | head -30
...
# Restore
cp scripts/bench/benchGraders.mjs.test_backup scripts/bench/benchGraders.mjs
...
echo "Running tests with mutation..."
echo ""
node scripts/mutation/mutation_test.mjs 2>&1 | grep -E "(Test [0-9]|Input:|fact_|abstained=)" | head -20

echo ""
echo "Checking if abstained flag is present..."
if node scripts/mutation/mutation_test.mjs 2>&1 | grep -q "abstained=true"; then
...
echo "Running tests with mutation..."
echo ""
node scripts/mutation/mutation_test.mjs 2>&1 | grep -E "(Test [0-9]|Input:|fact_|PASS|FAIL|FOUND|NOT_FOUND)" | head -30
...
echo "Running tests with mutation..."
echo ""
node scripts/mutation/mutation_test.mjs 2>&1 | grep -E "(Test [0-9]|Input:|fact_|PASS|FAIL)" | head -20
...
echo "--------------------------------------------------"
node scripts/mutation/mutation_test.mjs 2>&1 | tail -10
...
# Cleanup
rm -f scripts/bench/benchGraders.mjs.test_backup src/rules/entityContainment.js.test_backup
rm -f /tmp/mutate*.mjs
