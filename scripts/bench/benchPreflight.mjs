 ...
 *
 * Usage:
 *   node scripts/bench/benchPreflight.mjs              # check all models
 *   node scripts/bench/benchPreflight.mjs --env MODEL  # print env vars for download step
 */
...
if (args[0] === "--env") {
  ...
  if (!modelId) {
    console.error("Usage: node scripts/bench/benchPreflight.mjs --env <modelId>");
    process.exit(1);
  }
