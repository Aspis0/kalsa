...
Driving it: `scripts/device/device-share-send.sh` (`kalsa://share?text=`) — the type path is a no-op on a
...
- **CI**: `.github/workflows/bench.yml`, phases `fase0 | fase4 | smoke | tools | mem`, arms as a seed
  matrix. Knobs are AsyncStorage prefs written by `scripts/ci/ci-bench.sh` with a both-branch assert, and
