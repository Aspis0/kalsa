...
- Owner follow-up: `conversationActions.ts:248-262` builds each row's actions; `conversationRowActions.ts:4-26` binds Export and the existing delete confirmation to that row id; `HostRoot.tsx:231` shares the selected row's saved history. No rename implementation exists in `src`, so the sheet offers Esporta and Elimina only, as instructed.
- Preview binding text corrected at `ShellPreview.tsx:11,147`.
- Owner follow-up commands: `npx tsc --noEmit > /tmp/kalsa-ux-final-tsc.log 2>&1; code=$?; echo "EXIT=$code"; tail -30 /tmp/kalsa-ux-final-tsc.log; exit "$code"` EXIT=0; `npx jest --silent > /tmp/kalsa-ux-final-jest.log 2>&1; code=$?; echo "EXIT=$code"; tail -30 /tmp/kalsa-ux-final-jest.log; exit "$code"` EXIT=0 (167 suites / 2053 tests).
...
- Final audits: branch `ux-2026-09-21`, HEAD `a7ea2c7`; `git diff --check` EXIT=0; ratchets remain Shell 203/342, HostChat 303/350, HostRoot 241/241; worktree remains uncommitted.
- Could not verify: device build/install and captures; the orchestrator will run them. No engine, governor, pins, Meter, `AppShell.tsx`, or `AiChatPage.tsx` changes; `HostRoot.tsx:231` is the requested row-id export handoff.
