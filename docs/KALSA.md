...
| B — KEXP, **`use_mmap=false`** | S23 | 16/16, **21.3 → 15.7, monotone −26 %** | plan complete; 1 222 k majflt, battery 56 → 47 % |
...
| **2026-08-23** | ⭐ **The held experiment RESOLVED, and it went the recipe's way: the KEXP's collapse was EVICTION, not size (§7.45).** `use_mmap=false`, same phone, same 16 turns, one flag: **21.3 → 15.7 tok/s monotone** against **11.1 → 0.4** bimodal; whole plan in 464 s instead of 3055 s, 338 mAh instead of 904; no kill either way. **So the `ModelRegistry.ts` entry STAYS — the removal written on 2026-08-23 is discarded, not deferred.** ⚠️ This reverses the SPEED half of the drop decision above, and only that half: the artefact still fails the quality gate (+0.0705 macro bpb, zh +0.1053) and still answers in the wrong language about a third of the time, which is on its own sufficient to keep it off the shipping tiers. **What changed is the diagnosis, not the tier list.** The block is now named and buildable: q2_K/q3_K have no ARM repack path, so a NEON repack kernel for those two types reaches this arm's residency *without* surrendering mmap for the whole model — relayed to the engine team as the evidence for their queued item #29. ⛔ Also retired here: `majflt` as the thrash metric — the **fast** arm took **2.3× more** major faults, because an anonymous fault is a zram decompression and a file fault is a UFS read |
