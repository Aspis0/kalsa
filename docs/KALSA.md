...
| 2026-08-21 | **Model quality measured for the first time, and it pushes against both 8B tiers** (HARNESS_FINDINGS §7.38; 24 cells, 44 prompts in 4 languages, criteria fixed before any model ran, exact paired McNemar, every figure regenerated by `scripts/quality/analyse.mjs`). **LFM2.5-2.6B beats KEXP** in three independent configurations — 10–2 (p=0.039), 11–1 (p=0.006), 9–1 (p=0.021) — beats 8B-A1B in most comparisons and ties in one, and **answers in the question's language essentially always (0–2 of ~27) where the 8B tiers fail about a third of the time**. It is also 1.67 GB against 3.33 and shares its backbone with the VL-3B already chosen as the quality tier. **What KEXP still buys is speed**: 848 MB/token against 1666, 39 s against 56 s per median answer on the Jelly. **Not a decision — two measurements are missing and neither is another quality run:** the 2.6B has never been run on an S23, and **LFM2.5-VL-3B is still absent from `ModelRegistry.ts`**. **Qwen3.5-2B ties the 2.6B** (7–6, p=1.000), so the competitor is level with our quality tier, but it hit the 8192-token ceiling on 44 of 44 questions when run uncapped. |
