...
- Documents: **MOVED** import/extraction, helper functions, list row, empty state and progress overlay to the files above; **DEAD** old visual skin; **STILL MOUNTED** library routing, reorder, delete and detail entry in `DocumentsScreen.tsx:102-224,246-345`, detail/cover/snippet/rebuild/delete in `DocumentDetailView.tsx:47-276`.
...
- Rendered and inspected mock `?s=menu`, `?s=settings`, and `?s=switch` at the requested viewport. Each command used `"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --headless=new --disable-gpu --hide-scrollbars --window-size=349,621 --force-device-scale-factor=3 --screenshot=/tmp/kalsa-s3-mock-$state.png "file:///Users/marco/Projects/kalsa-ux/docs/design/kalsa-mock-v2.html?s=$state"`; **EXIT=0** for all three. Chrome emitted non-fatal macOS `CVDisplayLinkCreateWithCGDisplay` stderr.
...
- Final `npx jest --silent > /tmp/kalsa-s3-final-jest.log 2>&1; echo "EXIT=$?"; tail -30 /tmp/kalsa-s3-final-jest.log` — **EXIT=0**, 168 suites / 2,065 tests passed.
- `git diff --check > /tmp/kalsa-s3-final-diff-check.log 2>&1; echo "EXIT=$?"; tail-20 /tmp/kalsa-s3-final-diff-check.log` — **EXIT=0**. Source patch SHA-256 (tracked diff plus six new source files, excluding this report): `fbe02343c32d4a0c07407119520fc8863a5b4509239888bc7d5ff434b6659416` (`shasum -a 256 /tmp/kalsa-s3-source.patch`, **EXIT=0**).
- Could not verify the new native screens with an app/device capture; no app build or install was run. `docs/captures/2026-09-24/` contains only settings-named frames, so the described mistaken Account frame was not available there for a direct comparison.
