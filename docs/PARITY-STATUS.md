...
**Walk of the share-in + mini-app slice** (dirty tree on top of `437a57d`, not committed):
verdict moves **3, 4, 28 → IMPLEMENTED**, **41 → DIFFERENT (was MISSING)**,
**D2:16 → NOT REPRODUCED → REPRODUCED**; Table 3 gaps **5 and 6 resolved**. Unlike the walks
above, this slice's own claims were EXECUTED: `npx tsc --noEmit` exit 0 and `npx jest --silent`
exit 0 at **144 suites / 1,809 tests** (baseline at `437a57d`: 140 / 1,779, both exit 0). One
existing test changed: `notesNotice.test.ts` "the host gives the send the slot it asks for" now
pins the root's destructure WITH `showNotice` — the raw-string writer the mini-app sheet's block
actions need (old `App:5346-5362` passed the same one); its one-writer assertion is untouched.
New tests: `shareIn.test.ts`, `shareImport.test.ts`, `hostOverlay.test.ts`, `miniappHistory.test.ts`
(30 tests). Every verdict below this paragraph is still only true of the tree that produced it —
re-walk after the next slice.
...
| 3 | Eight exclusive overlays | **IMPLEMENTED** — CHANGED this walk (was DIFFERENT: `miniapp` kind deleted) | 8 of 8 kinds: `src/host/hostOverlay.ts:4-27` — the `miniapp` kind restored at `:25`, the deletion's reason kept in the header, the open policy `withMiniappOverlay` at `:39-44` (old `App:7035-7046`); mounts `src/host/HostOverlays.tsx` — the sheet branch `:269-277` renders `src/host/HostMiniappSheet.tsx` — assembled by `HostFurniture.tsx:66-92`. |
| 4 | Mini-app card **and** full-screen sheet | **IMPLEMENTED** — CHANGED this walk (was MISSING) | Card: field `transcriptTypes.ts:73` + mapper read `messageMapper.ts:118` → `src/ui/shell/MiniappCard.tsx`, rendered in the answer before the sources (`TranscriptTurns.tsx:230-235`, old `Chat:5665`); opener thread `Transcript.tsx:280` → `HostChatSurface.tsx:174` → policy `HostRoot.tsx:218` (old prop chain `App:1004,7035-7046`). Sheet: `HostMiniappSheet.tsx` — the controller's chrome (`App:7214-7252`) with a 48 dp close box instead of `hitSlop={8}` — around the CALLED renderer; block actions `:99-112` = old `App:5346-5362`. Plumbing now USED, not just present: persist `historyMessages.ts:128-143`, capture `sendCallbacks.ts:125-133`, per-send hook `engineTurn.ts:103`. Tests: `hostOverlay.test.ts`, `miniappHistory.test.ts`. **Held/declared:** `setAskAssistantDraft` is the controller's own no-op (`App:5347`; `miniappActions.ts` never calls it in either build); a card with no opener draws neither the "tap to open" hint nor the Open control. |
...
| 28 | Mini-app card inside a message | **IMPLEMENTED** — CHANGED this walk (was MISSING) | Same evidence as row 4's card half: `MiniappCard.tsx`, mounted at `TranscriptTurns.tsx:230`; pinned by `miniappHistory.test.ts` (save → disk → restore → save byte-stable, mapper carry, text-only migration) and `hostOverlay.test.ts` (open policy). Old `Chat:6033-6093`. |
...
| 41 | Share-in: prefill, file imports, notices | **DIFFERENT** — CHANGED this walk (was MISSING) | Parse + merge CALLED from `src/app/shareIntent.ts` (untouched): `src/host/useShareIn.ts` carries the listener `:83-98` (old `App:3638-3660`), the pending flush `:103-108` (old `:3661-3671`) and the nonce merge `:110-119` (old `Chat:1959-1969`); the gate is `src/host/shareIn.ts:51-82` (dedupe + pending = old refs `App:3559-3562`), the file branch `src/host/shareImport.ts:47-102` (caps + busy/too-large/failed = old `App:3583-3637`); root wiring `HostRoot.tsx:131` (draft/drawer/notice/library ports). **How different:** the PDF half imports into Documents and then **holds the composer auto-attach** — this build has no attachment flow (row 43) — announcing it with `errors.shareImportNotAttached` (`en.ts:755`, `it.ts:749`) where the controller set `shareAttachDoc` (`App:3616-3621` → `Chat:3717-3722`). Tests: `shareIn.test.ts`, `shareImport.test.ts`. |
...
**Tally:** IMPLEMENTED 18 (3, 4, 8, 9, 10, 13, 20, 21, 22, 23, 24, 25, 27, 28, 39, 42, 44, 47) · DIFFERENT 15 (1, 2, 5, 6, 7, 11, 12, 14, 15, 16, 33, 35, 37, 41, 45) · STUBBED 2 (29, 31) · MISSING 13 (17, 18, 19, 26, 30, 32, 34, 36, 38, 40, 43, 46, 50) · nothing-to-preserve 2 (48, 49). 18 + 15 + 2 + 13 + 2 = 50.
...
**Verdicts moved by the share-in + mini-app slice:** 3, 4, 28 (→ IMPLEMENTED), 41 (MISSING → DIFFERENT), D2:16 (NOT REPRODUCED → REPRODUCED); gaps 5–6 resolved.
...
| 16 | Share-in | **REPRODUCED** — CHANGED this walk (was NOT REPRODUCED; export-out exists, import-in did not). | Gate `shareIn.ts:51-82` + wiring `useShareIn.ts:83-119`: consume-once, pending until ready, nonce re-merge, busy/too-large/failed notices. | Check: `shareIn.test.ts` (same URL twice → one claim; held URL applied exactly once by the flush; identical text twice merges twice, draft kept) + `shareImport.test.ts` (each notice fires; both catalogues carry every key). |
...
| 5 | ~~**Mini-app card + sheet unreachable**~~ **RESOLVED this walk** | — (card + sheet + policy landed) | old `Chat:6033-6093`, `App:7214-7252` | done: `MiniappCard.tsx` + `HostMiniappSheet.tsx` + `hostOverlay.ts:25,39` | rows 4, 28 → IMPLEMENTED |
| 6 | ~~**Share-in dead**~~ **RESOLVED this walk** (PDF auto-attach held → row 41 DIFFERENT) | — (text/txt/md prefill + PDF import + notices landed) | old `App:3553-3671`, `shareIntent.ts:37` | done: `useShareIn.ts` + `shareIn.ts` + `shareImport.ts` | row 41 → DIFFERENT |
...
research/notes chips + quick templates, root size ratchet. **Resolved by the share-in + mini-app slice:** gaps 5 and 6 (evidence in rows 3, 4, 28, 41 and D2:16).
...
| 5 | Test results for every walk BEFORE the share-in + mini-app slice — those claims remain existence-only; this slice's claims were executed (144 suites / 1,809 tests, both gates exit 0) | re-run `npx jest --silent` to refresh |
