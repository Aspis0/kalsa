...
- **QAD-Q4_0 is better than free on the 2.6B**: same 34/44 at 5 % fewer bytes (3–3, p = 1.000) and
  **+6.9 % decode measured on the Jelly** (8.63 vs 8.07 t/s, `scripts/device/device-decode-lineup.sh`).
...
`HONESTY_PATTERNS` (`scripts/bench/benchGraders.mjs:34-48`) is a hardcoded list of Italian regexes —
...
1. **`run-as` needs a debuggable build, and today's device APK is a release build.** `apk.yml`
   ...
   *verification*: it never ran `scripts/native/assert-native-patch.sh`, so nothing proved the marker
...
`scripts/device/device-share-send.sh` (`kalsa://share?text=`).
...
cells. Everything below is regenerated by `node scripts/bench/quality/analyse.mjs`; no figure here was
transcribed by hand, because this file has been burned by transcribed figures before.

**Method.** `scripts/bench/quality/questions.json` holds the prompts and the pass criteria together, so
...
(`scripts/device/device-decode-lineup.sh`):
...
3. **A dormant privacy defect gets armed.** §7.7 above closes with "with thinking off the emitted
   ...
   reasoning in plaintext for the life of the conversation, and `scripts/ci/ci-dflash-ab.sh:235-238`
...
exception worth fixing: `scripts/ci/ci-dflash-ab.sh:235-238` dumps the whole message row into bench
