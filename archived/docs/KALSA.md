...
| 2026-09-09 | **The practical plan starts and the app stops waiting for the model choice** (`docs/PROPOSTA-OTTIMIZZAZIONE-CHAT-2026-09-08.md`; HARNESS_FINDINGS §7.41-7.45). Repo consolidated to `main` only (17 `archive/*` tags). Shipped on main today, each from a measurement: language rule "the language the user writes in" (f77ea7b); PDF text extraction no longer capped at 5 pages, lenient document selection, correct SEND filters, cold-start share parked until hydration, 180 s extraction timeout with partial text (5707cfe); opt-in `dev-minicpm5-2b` with one-time sha256 (7836db7). Measured: restore 37.9 s → 562 ms on the S23 (n=1); document chat 1/5 → 7/7 on the Jelly at 2.8-3.7 tok/s; G1 Mac blind judging near parity once three harness artefacts were removed; MiniCPM5 decodes 6.5 vs LFM 10.4-11.7 tok/s on the S23 and its prewarm ingests tokens one at a time (open). Still open with numbers: tool turns make the session unsaveable (cold reopen after any document chat); full_context strategy ignores the phone's prefill budget. |
