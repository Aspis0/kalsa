/** Small synchronous SHA-256/HMAC implementation for the pairing byte contract. */
...
export function sha256(input: Uint8Array): Uint8Array {
  ...
  for (let index = 0; index < 8; index += 1) {
    padded[paddedLength - 1 - index] = remainingBits % 256;
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const small0 = rotr(7, x) ^ rotr(18, x) ^ (x >>> 3);
      const small1 = rotr(17, y) ^ rotr(19, y) ^ (y >>> 10);
    ...
    for (let index = 0; index < 64; index += 1) {
      const big1 = rotr(6, e) ^ rotr(11, e) ^ rotr(25, e);
      const choose = (e & f) ^ (~e & g);
      const temp1 = (h + big1 + choose + ROUND[index] + words[index]) >>> 0;
      const big0 = rotr(2, a) ^ rotr(13, a) ^ rotr(22, a);
...
export function hmacSha256(key: Uint8Array, message: Uint8Array): Uint8Array {
  ...
  const normalizedKey = key.length > block.length ? sha256(key) : key;
