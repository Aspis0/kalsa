...
fn exe_for_uses_the_resolved_processor_build() {
    ...
    let mut processor = Some(PathBuf::from("/stub-cpu"));
    let exe = exe_for(
        &candidate,
    ...
    assert_eq!(processor, Some(PathBuf::from("/stub-cpu")));
}
...
fn a_refused_tune_is_saved_and_the_rule_stands() {
    ...
    let mut memo = Memo {
        cores: CORES,
        processor: Some(PathBuf::from("/stub-cpu")),
    };
    ...
    tune_launch(
        ...
        &mut memo,
        &mut progress,
        |candidates, _, _, counts: &mut dyn FnMut(usize, usize)| {
            counts(candidates.len(), candidates.len());
            candidates
                .iter()
                .map(|candidate| (*candidate, kalsa_tune::Outcome::Refused(kalsa_tune::Refusal::NotReady)))
                .collect()
        },
    );

    assert!(
        matches!(prepared.info.tune, Some(Tune::NoWinner)),
        "nothing won: {:?}",
        prepared.info.tune
    );
    let argv = &prepared.server.argv;
    ...
    assert!(
        seen.iter().any(|step| matches!(step, Progress::Tuning { done: 3, planned: 3 })),
...
fn the_tune_line_is_the_owners_copy() {
    ...
    assert_eq!(
        ...
        "graphics card, 49.0 tokens/s (processor 16 threads: 11.8 tokens/s)"
    );
    ...
    assert_eq!(tune_line(&Tune::Measured(alone)), "graphics card, 49.0 tokens/s");
}
