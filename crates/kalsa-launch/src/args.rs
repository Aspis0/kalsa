...
pub enum Offload {
    ...
    All,
    /// CPU decode, stated with `--n-gpu-layers 0`: either the budget was
    /// not sized for the card (an unsized full offload is how a card OOMs),
    /// or it is the processor baseline the tune measures against.
    ForcedOff,
