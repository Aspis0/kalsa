...
mod tests {
        ...
        for model in CATALOG.iter() {
            for gib in [8u64, 16, 32, 64] {
                ...
                assert!(
                    ...
                    "{} on {gib} GiB: context {} tokens, roof {roof_mib} MiB,                      reserved {reserved} over usable {}",
                    model.display_name,
    ...
    fn the_measured_cache_figure_sizes_the_context_where_the_assumption_undercounted() {
        ...
        // and 512 MiB of buffers leave 7_279_969_567 bytes; the
        // sleeping-chat reserve takes a quarter — 1_819_992_391 bytes —
        // and the context funds the rest: 5_459_977_176 bytes at 163_840
        // bytes per token = 33_325 whole tokens.
        let model = shipped_row(APERTUS);
        let budget = memory_budget(Backend::Cpu, 64 * GIB);
        let launched = plan(&input(ServerBackend::Cpu, budget, model, M1_MAX_RAMP))
            .expect("the model is fundable");
        assert_eq!(launched.args.context_tokens, 33_325);
        assert!(fits(model, launched.args.context_tokens, &budget));
        assert_eq!(launched.memory.kv_cache_bytes, 33_325 * 163_840);
        assert!(
            !launched.memory.kv_per_token_assumed,
