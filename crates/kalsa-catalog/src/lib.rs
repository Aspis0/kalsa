...
//! * **a download is an address, not a guess — and the menu proves it** —
//!   the chooser only ever sees rows of the download table, whose type
//!   cannot be written without the exact file at a pinned commit, its size
//!   and the sha256 every byte is verified against. A researched row with
//!   no identified file stays in the research record, where it can be
