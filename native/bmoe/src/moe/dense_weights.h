...
// own (DenseWeightsMode): left mmap'd, mmap'd-and-warmed, read once into anonymous buffers via
// O_DIRECT and rebound, or AnonGpu (same copy, but skip tensors already in a non-host buffer).
// This module owns that policy, the buffers it may allocate, and the residency
...
public:
    ...
    // `tensors` needed only for Anonymous/Pinned/AnonGpu). `align` is the O_DIRECT block size. Runs
    // once at load, before the streamer's workers start (Anonymous rebinds tensor->data on the
    // caller's thread). GPU-resident tensors are never rebound — they stay on the device buffer and
    // only their leftover mmap pages are dropped. Returns false only on a hard Anonymous failure
    // (alloc/read); Mmap and Warmed cannot fail.
