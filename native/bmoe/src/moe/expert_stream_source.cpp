...
bool ExpertStreamSource::init(const std::vector<std::string> & shard_paths,
    ...
    if (cfg.cache_auto) {
        ...
        if (dense_mode_host_copy(cfg.dense_weights)) {
            uint64_t dense_pending = 0;
            for (const DenseTensorRef & d : dense_tensors_) {
                if (!d.tensor) continue;
                // GPU-resident dense is already in DRAM (CL buffer). MemAvailable typically
                // already counts it; deducting it again would plan a host twin we will not allocate.
                if (d.tensor->buffer && !ggml_backend_buffer_is_host(d.tensor->buffer)) continue;
                // Routed experts are streamed, not converted to a dense host twin.
                if (d.tensor->name[0]) {
                    const char * n = d.tensor->name;
                    if (std::strstr(n, "shexp") == nullptr && std::strstr(n, ".ffn_") != nullptr &&
                        std::strstr(n, "exps") != nullptr)
                        continue;
                }
                dense_pending += d.size;
            }
    ...
    // Expert tensors were just rebound off the file mmap onto slot/cache buffers. Capture
    // warm-up may still have left those expert pages resident in page cache with nobody
    // holding a live pointer into them — orphan pressure that competes with the dense anon
    // conversion and the expert cache. Symmetric to DenseWeights::drop_mmap_copies: map each
    // expert range back to its VMA via /proc/self/maps and MADV_DONTNEED. Best-effort; does
    // not reduce I/O already spent, only the orphaned cache footprint.
    {
        std::vector<std::string> basenames;
        basenames.reserve(shard_paths.size());
        for (const std::string & p : shard_paths) {
            const size_t slash = p.find_last_of("/\\");
            basenames.push_back(slash == std::string::npos ? p : p.substr(slash + 1));
        }
        std::vector<std::vector<pio::MappedRegion>> vmas(basenames.size());
        for (size_t s = 0; s < basenames.size(); ++s)
            pio::file_mapped_regions(basenames[s].c_str(), vmas[s]);
        auto addr_of = [&](int file_idx, uint64_t off) -> const char * {
            if (file_idx < 0 || file_idx >= (int) vmas.size()) return nullptr;
            for (const auto & v : vmas[(size_t) file_idx]) {
        ...
        const size_t page = page_ ? page_ : pio::vm_page();
        uint64_t dropped = 0;
        for (const LayerExperts & L : layers_) {
            if (!L.bound) continue;
            for (int p = 0; p < MoeRecipe::max_exps; ++p) {
                if (!L.proj[p].tensor || L.proj[p].nb2 == 0) continue;
                const uint64_t sz = (uint64_t) L.proj[p].nb2 * (uint64_t) n_expert_;
                const char * a = addr_of(L.proj[p].file_idx, L.proj[p].file_off);
                if (!a || sz == 0) continue;
                // Align INWARD so a page shared with an adjacent dense tensor is never dropped.
                uintptr_t a0 = ((uintptr_t) a + page - 1) & ~(uintptr_t) (page - 1);
                uintptr_t a1 = ((uintptr_t) a + sz) & ~(uintptr_t) (page - 1);
                if (a1 > a0) {
        ...
        std::fprintf(stderr, "bmoe: expert-mmap: dropped %llu MiB\n",
