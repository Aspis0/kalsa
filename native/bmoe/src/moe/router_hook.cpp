...
// Isolation (cb_eval ask=true) syncs the backend but does not copy OpenCL outputs onto a
// real host pointer. ggml-opencl's get_base is a dummy alignment address, so t->data on a
// GPU topk/weights node is not dereferenceable. ARGSORT is supported for n_expert=256, so
// a GPU trunk places ffn_moe_topk on OpenCL. The streamer and drop policy still read/write
// through t->data — materialize a host alias for the duration of the callback.
static ggml_backend_buffer_t tensor_storage_buffer(const ggml_tensor * t) {
    if (!t) return nullptr;
    return t->view_src ? t->view_src->buffer : t->buffer;
}

static bool tensor_on_host(const ggml_tensor * t) {
    ggml_backend_buffer_t buf = tensor_storage_buffer(t);
    return t && t->data && buf && ggml_backend_buffer_is_host(buf);
}
...
struct HostAlias {
    ...
    HostAlias(ggml_tensor * tensor, bool want_write) : t(tensor), write(want_write) {
        if (!t) return;
        saved = t->data;
        if (tensor_on_host(t)) return;
        const size_t n = ggml_nbytes(t);
        if (n == 0 || !tensor_storage_buffer(t) || !t->data) return;
        buf.resize(n);
        ...
        if (!logged) {
            logged = true;
            std::fprintf(stderr,
                         "bmoe: routing tensor '%s' is not host-backed — copying ids/weights "
                         "through the backend (GPU trunk + CPU streamer)\n",
                         t->name);
        }
    }
    ...
    ~HostAlias() {
        if (!t) return;
        if (copied && write && !buf.empty()) ggml_backend_tensor_set(t, buf.data(), 0, buf.size());
        t->data = saved;
    }

    HostAlias(const HostAlias &) = delete;
    HostAlias & operator=(const HostAlias &) = delete;
};
...
void RouterHook::apply_drop(ggml_tensor * wt) {
    ...
    // Caller already aliased `wt`. The ids live on the isolated topk node (often GPU).
    HostAlias ids_host(D.ids, /*want_write=*/true);
...
bool RouterHook::on_eval(ggml_tensor * t, bool ask) {
    ...
    if (is_weights && t->data && t->type == GGML_TYPE_F32) {
        const bool on_drop_layer = drop_.layer == wl;
        const bool will_write =
            on_drop_layer && drop_.deferred && wl >= 0 && wl < (int) term_variant_.size() &&
            term_variant_[wl] == weight_variant;
        HostAlias wt_host(t, will_write);
        if (pending_.layer == wl && pending_.nu > 0)
            gather_weights(t, pending_.nu, pending_.nt, pending_.weights);
        // Learn which node ends this layer's weight chain, and — once known — use it as the
        // point where the routing is decided. The learning pass and the deferral are the same
        // walk, so a layer whose chain shape the hook has not seen yet keeps undropped behaviour.
        if (on_drop_layer) {
            chain_last_ = (int8_t) weight_variant;
            if (will_write) apply_drop(t);
        }
    }

    if (source_ && is_topk && t->data && t->type == GGML_TYPE_I32) {
        // Isolation syncs OpenCL but leaves t->data as a dummy pointer. Alias first so
        // gather / route-ahead / prev_ids read real ids. apply_drop aliases this tensor later.
        HostAlias topk_host(t, route_ahead_ > 0 && batch_phase_ == 1);
